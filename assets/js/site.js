...
function imageUrl(v,f='assets/img/storm-logo.svg',width){
  ...
  // SPEED: Storm HQ stores the original uploaded Google Drive image URL in the
  // Sheet. For display only, request a smaller Drive thumbnail sized for the
  // place it is being shown. The original Drive file and Sheet value are never changed.
  try{
    const u=new URL(raw);
    const host=u.hostname.replace(/^www\./,'').toLowerCase();
    if(host==='drive.google.com' && u.pathname==='/thumbnail' && u.searchParams.get('id')){
      u.searchParams.set('sz','w'+Math.max(120,Math.round(Number(width)||1200)));
      return u.toString();
    }
  }catch(e){}
  return raw;
