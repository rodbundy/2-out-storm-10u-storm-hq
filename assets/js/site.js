...
function imageUrl(v,f='assets/img/storm-logo.svg'){return val(v,f);}
...
function renderReports(){const s=$('#shoutouts');if(s)s.innerHTML=arr('shoutouts').map(announcementCard).join('')||'<div class="empty-state">No Storm Reports yet.</div>';const g=$('#gallery');if(g)g.innerHTML=arr('gallery').map(p=>`<article class="gallery-card"><img src="${esc(imageUrl(p.ImageURL))}" alt="${esc(p.Title||'Storm photo')}"><div><h3>${esc(p.Title||'Inside the Storm')}</h3><p>${esc(p.Caption||'')}</p></div></article>`).join('')||'<div class="empty-state">Storm Gallery coming soon.</div>';}
