...
const DEVICE_KEY='stormhq_push_device_id_v1';
const coachMode=new URLSearchParams(location.search).get('coach')==='1';
const SESSION_KEY=coachMode?'stormhq_push_coach_session_v1':'stormhq_push_family_session_v1';
...
function render(){
  ...
  const coachLaunch=!coachMode?`<div class="storm-push-card" style="grid-column:1/-1"><span class="eyebrow">Coach Access</span><h3>Coach Device Setup</h3><p class="storm-note">If you generated a temporary coach pairing code in Coach HQ, pair this installed Storm HQ app as a coach device here.</p><a href="storm-notifications.html?coach=1" class="button orange storm-phone-button">Pair This Phone as Coach</a></div>`:'';
  host.innerHTML=installCard+`<div class="storm-push-card"><div class="storm-push-status"><i class="storm-push-dot ${configured?'good':'warn'}"></i>${configured?'Push System Ready':'Coach Setup In Progress'}</div><h3>Phone Notifications</h3><p class="storm-note">Permission: <b>${esc(perm)}</b>. Firebase: <b>${configured?'connected':'not connected yet'}</b>.</p></div>`+(session?settingsCard():login)+coachLaunch;
