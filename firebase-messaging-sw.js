/* 2 OUT STORM HQ — PWA + FIREBASE MESSAGING SERVICE WORKER
   SPEED RULE: cache the shell, never intercept the Apps Script API, and update static files in the background. */
const STORM_CACHE='storm-hq-shell-v1';
const STORM_CORE=[
  '/', '/index.html', '/offline.html', '/manifest.webmanifest',
  '/assets/css/styles.css', '/assets/css/storm-pwa.css',
  '/assets/js/config.js', '/assets/js/api.js', '/assets/js/site.js',
  '/assets/js/push-config.js', '/assets/js/storm-pwa.js',
  '/storm-notifications.html',
  '/assets/img/storm-logo.svg',
  '/assets/img/icons/icon-192.png','/assets/img/icons/icon-512.png','/assets/img/icons/icon-512-maskable.png','/assets/img/icons/badge-96.png'
];
...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    await Promise.all(keys.filter(k=>k.startsWith('storm-hq-shell-')&&k!==STORM_CACHE).map(k=>caches.delete(k)));
    await clients.claim();
  })());
});
...
async function networkFirst(req){
  ...
  try{
    ...
    const res=await fetch(req,{signal:ctrl.signal}); clearTimeout(timer);
...
self.addEventListener('fetch',event=>{
  ...
  // Never put private/public Apps Script API calls behind the service-worker cache.
...
// Firebase is loaded only in the worker and only after the coach enables the config.
try{
  importScripts('/assets/js/push-config.js');
  ...
  console.warn('[Storm HQ] Push worker waiting for Firebase setup.',err?.message||err);
}
